...
('10th Marksheet', 'app/static/uploads/10th-marksheet.pdf', 'Academic',
    (SELECT department_id FROM departments WHERE department_name = 'Computer Engineering'),
    'Approved', 'Verified and matched with admission records.', '2022CE041', NULL),
('12th Marksheet', 'app/static/uploads/12th-marksheet.pdf', 'Academic',
    (SELECT department_id FROM departments WHERE department_name = 'Computer Engineering'),
    'Pending', 'Upload a clearer scan with visible board stamp.', '2022CE041', NULL),
('Leaving Certificate', 'app/static/uploads/leaving-certificate.pdf', 'Academic',
    (SELECT department_id FROM departments WHERE department_name = 'Computer Engineering'),
    'Approved', 'Accepted by the admin office.', '2022CE041', NULL),
('Placement Resume', 'app/static/uploads/riya-placement-resume.pdf', 'Placement',
    (SELECT department_id FROM departments WHERE department_name = 'Placement Cell'),
    'Pending', 'Please update your resume headline and skills section.', '2022CE041', NULL),
('GitHub Profile Link', 'https://github.com/riya-patel-dev', 'Placement',
    (SELECT department_id FROM departments WHERE department_name = 'Placement Cell'),
    'Approved', 'Profile reviewed and accepted by placement mentor.', '2022CE041', NULL),
('LinkedIn Profile Link', 'https://www.linkedin.com/in/riya-patel-dev', 'Placement',
    (SELECT department_id FROM departments WHERE department_name = 'Placement Cell'),
    'Pending', 'Please add headline, skills and latest projects before final review.', '2022CE041', NULL),
('Scholarship Form', 'app/static/uploads/scholarship-form-2026.pdf', 'Scholarship',
    (SELECT department_id FROM departments WHERE department_name = 'Scholarship Cell'),
    'Pending', 'Fee receipt still under verification.', '2022CE041', NULL),
('Fee Receipt', 'app/static/uploads/fee-receipt.jpg', 'Finance',
    (SELECT department_id FROM departments WHERE department_name = 'Scholarship Cell'),
    'Approved', 'Payment proof confirmed.', '2022CE041', NULL),
('Department Report', 'app/static/uploads/department-report-cse-april.pdf', 'Department',
    (SELECT department_id FROM departments WHERE department_name = 'Placement Cell'),
    'Approved', 'Shared with admin dashboard.', NULL,
    (SELECT teacher_id FROM teachers WHERE email_id = 'teacher@institution.edu')),
('Attendance Sheet', 'app/static/uploads/attendance-sheet.pdf', 'Academic',
    (SELECT department_id FROM departments WHERE department_name = 'Computer Engineering'),
    'Pending', 'Teacher should upload latest monthly attendance.', NULL,
    (SELECT teacher_id FROM teachers WHERE email_id = 'teacher@institution.edu'));

INSERT INTO notifications (prn, document_id, message) VALUES
('2022CE041', 2, 'Upload a clearer scan with visible board stamp.'),
('2022CE041', 4, 'Please update your resume headline and skills section.'),
('2022CE041', 7, 'Fee receipt still under verification.');
